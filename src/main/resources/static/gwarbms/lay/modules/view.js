    ...
    .define(
        ...
        function (exports) {
            ...
            self.loading = function (elem) {
                elem.append(
                    (this.elemLoad = $(
                        '<i class="layui-anim layui-anim-rotate layui-anim-loop layui-icon layui-icon-loading gwarbms-loading"></i>'
                    ))
                )
            };
            ...
            self.modal.base = function (msg, params) {
                ...
                params = $.extend(
                    {
                        skin: 'layui-layer-admin-modal gwarbms-alert',
            ...
            self.tab = {
                ...
                minLeft: null,
                maxLeft: null,
                wrap: '.gwarbms-tabs-wrap',
                menu: '.gwarbms-tabs-menu',
                next: '.gwarbms-tabs-next',
                prev: '.gwarbms-tabs-prev',
                step: 200,
                init: function () {
                    var tab = this;
                    var btnCls = tab.wrap + ' .gwarbms-tabs-btn';

                    layui.dropdown.render({
                        elem: '.gwarbms-tabs-down',
                        click: function (name) {
                            if (name === 'all') {
                                tab.delAll();
                            }
                            ...
                            if (name === 'current') {
                                tab.del(layui.gwarbms.route.fileurl);
                            }
                            if (name === 'refresh') {
                                tab.refresh();
                            }
                    ...
                    $(document).on('click', btnCls, function (e) {
                        ...
                        if ($(e.target).hasClass('gwarbms-tabs-close')) {
                            tab.del(url)
                        } else {
                    ...
                    $('.gwarbms-tabs-hidden').addClass('layui-show');
                    this.isInit = true
                },
                has: function (url) {
                    var exists = false;
                ...
                delAll: function (type) {
                    var tab = this;
                    var menuBtnClas = tab.menu + ' .gwarbms-tabs-btn';
                ...
                delOther: function () {
                    var tab = this;
                    var menuBtnClas = tab.menu + ' .gwarbms-tabs-btn';
                    $(menuBtnClas + '.gwarbms-tabs-active')
                        .siblings()
                        .each(function () {
                            var url = $(this).attr('lay-url');
                            tab.del(url)
                ...
                del: function (url, backgroundDel) {
                    ...
                    var lay = '[lay-url="' + url + '"]';
                    var thisBody = $(
                        '#' + conf.containerBody + ' > .gwarbms-tabs-item' + lay
                    );
                    ...
                    if (backgroundDel === undefined) {
                        if (thisMenu.hasClass('gwarbms-tabs-active')) {
                            $(this.menu + ' li:last').click()
                        }
                    }
                },
                refresh: function (url) {
                    url = url || layui.gwarbms.route.fileurl;
                    if (this.has(url)) {
                        this.del(url, true);
                        self.renderTabs(url)
                    }
                ...
                clear: function () {
                    this.data = [];
                    this.isInit = false;
                    $(document).off('click', this.wrap + ' .gwarbms-tabs-btn')
                },
                change: function (route, callback) {
                    ...
                    var activeCls = 'gwarbms-tabs-active';
                    ...
                    if (existsTab) {
                        ...
                        layui.gwarbms.navigate(route.href)
                    } else {
                        self.loadHtml(fileurl, function (res) {
                            var htmlElem = $(
                                "<div><div class='gwarbms-tabs-item' lay-url='" +
                                fileurl +
                                "'>" +
                                res.html +
                                '</div></div>'
                            ...
                            tab.data.push(route);
                            layui.gwarbms.render(tab.tabMenuTplId);
                    ...
                    layui.gwarbms.sidebarFocus(route.href);
                    return false
                },
                onChange: function () {
                }
            ...
            self.renderLayout = function (callback, url) {
                ...
                self.render(url, function (res) {
                    ...
                    if (conf.viewTabs === true) {
                        self.containerBody.addClass('gwarbms-tabs-body')
                    }
                    layui.gwarbms.appBody = self.containerBody;
